        ...
        for E_num in range(E_size-1):
            ...
            for e in range(args.iter//E):
                ...
                for i in range(ntrain):
                    ...
                    tau = generate_trajectory_cliff(train_P[0], nS, nA, initial_state=0, max_steps=5)
                    for _ in range(E):
                        grad = softmax_policy_gradient(local_pi_logit, train_P[i], R, gamma)
                        P_pi = trajectory_probability_wiff(tau, train_P[i], local_pi_logit)
                        if len(pis_logit)!=0:
                            P_pi_pre = trajectory_probability_wiff(tau, train_P[i], pis_logit[-1])
                        else:
